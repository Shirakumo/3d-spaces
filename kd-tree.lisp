  ...
  (let ((dim-value (ecase axis
    ...
    (let* ((mid (truncate (length children) 2))
           (median (if (oddp (length children))
                       ...
                       (* 0.5f0 (+ (funcall dim-value (aref children (+ mid 0)))
...
(defun split-node (node dims split-size)
  ...
  (unless (= most-negative-single-float (node-position node))
    (let* ((children (node-children node))
           ...
           (min 0.0f0) (max 0.0f0) (max-range 0.0f0) (max-dim 0) (others ()))
  ...
  (with-array (c region)
    (with-array (b (region-size region))
      (let ((v (make-array 3 :element-type 'single-float))
        ...
        (setf (aref v 0) (incf (aref c 0) (setf (aref b 0) (* 0.5f0 (aref b 0)))))
        (setf (aref v 1) (incf (aref c 1) (setf (aref b 1) (* 0.5f0 (aref b 1)))))
        (setf (aref v 2) (incf (aref c 2) (setf (aref b 2) (* 0.5f0 (aref b 2)))))
        (flet ((visit (node)
                   ...
                   (let ((a (node-near node))
                     ...
                     (when a
                       (when (< position (aref o axis))
                         (rotatef a b))
                       (if (= 0.0f0 (aref d axis))
                           (visit a tmax)
                           (let ((tt (/ (- position (aref o axis)) (aref d axis))))
                             (cond ((and (<= 0.0f0 tt) (< tt tmax))
                                    (visit a tt)
